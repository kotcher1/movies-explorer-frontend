    ...
    <div className="card-list">
      {props.relevantMovies.length === 0 ? <NotFound message={props.message} /> : (
        <div className="card-list__container">
          {props.moviesState === 'all' && props.relevantMovies.map((card, index) => {
            if (index < currentIndex) {
              return <MoviesCard info={card} key={card.nameRU}/>
            }
          })}
          {props.moviesState === 'saved' && savedCards.map(card => {
            return <MoviesCard info={card} key={card.nameRU} delete/>
          })}
        </div>
      )}
